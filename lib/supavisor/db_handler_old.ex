defmodule Supavisor.DbHandlerOld do
  require Logger
  use GenServer
  ...
  def call(pid, msg) do
    GenServer.call(pid, {:db_call, msg}, 5000)
  end
  ...
  def init(args) do
    # IP
    # {:ok, host} =
    #   Application.get_env(:supavisor, :db_host)
    #   |> String.to_charlist()
    #   |> :inet.parse_address()
  ...
  def handle_info({:EXIT, caller, reason}, %{caller: caller} = state) do
    Logger.error("Client #{inspect(caller)} exited #{inspect(reason)}")

    Supavisor.get_local_pool(state.tenant)
  ...
  defp decrypt_password(password) do
    Application.get_env(:supavisor, :db_enc_key)
    |> Supavisor.Helpers.decrypt!(password)
  end
end
