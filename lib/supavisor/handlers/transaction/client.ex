defmodule Supavisor.Handlers.Transaction.Client do
  @moduledoc false
  ...
  alias Supavisor.Handlers.Transaction.Db, as: ProxyDb
    ...
    case handle_exchange(sock, {method, secrets}) do
      ...
      {:ok, client_key} ->
        ...
        {:keep_state, %{data | auth_secrets: {method, secrets}, auth: auth},
         {:next_event, :internal, {:client, :subscribe}}}
    end
  end
  ...
  def handle_event(:info, {proto, _, bin}, _, data) when proto in @proto do
    {db_pid, db_sock} =
      if is_nil(data.db_pid) do
        pid = :poolboy.checkout(data.pool, true, 90_000)
        Process.link(pid)
        {:ok, s} = :gen_statem.call(pid, {:tcp_owner, self()}, 5_000)
        Logger.debug("ProxyClient: Checkout new db connection #{inspect({pid, s})}")
        {pid, s}
      else
        {data.db_pid, data.db_sock}
      end
  ...
  def handle_event(_, {closed, _}, _, data)
    ...
    Logger.debug(
      "ProxyClient: #{closed} socket closed for #{inspect(data.tenant)} #{inspect(data.db_sock)}"
    )
