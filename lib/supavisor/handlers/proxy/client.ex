defmodule Supavisor.Handlers.Proxy.Client do
  ...
  def handle_event(:internal, {:client, :subscribe}, _, %{auth: auth} = data) do
  ...
  def handle_event(:timeout, :subscribe, _, _) do
    {:keep_state_and_data, {:next_event, :internal, {:client, :subscribe}}}
  end

  def handle_event(:timeout, :wait_ps, _, data) do
    msg = "ClientHandler: Wait parameter status timeout, send default #{inspect(data.ps)}}"
    Logger.error(msg)
  ...
  def log(level, message) when is_atom(level) and is_binary(message) do
    apply(Logger, level, ["ProxyClient: " <> message])
  end
