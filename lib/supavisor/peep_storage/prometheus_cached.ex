defmodule Supavisor.PeepStorage.PrometheusCached do
  @moduledoc """
  Vendored from Peep.Prometheus.Cached.
  ...
  def export_metrics({reverse_tags_tid, cache_tid, metrics}) do
    Enum.map(metrics, &format(&1, reverse_tags_tid, cache_tid))
  end

  defp format({%Counter{}, _series} = metric, reverse_tags_tid, cache_tid) do
    format_standard(metric, "counter", reverse_tags_tid, cache_tid)
  end

  defp format({%Sum{} = spec, _series} = metric, reverse_tags_tid, cache_tid) do
    format_standard(
      metric,
  ...
  defp format({%LastValue{} = spec, _series} = metric, reverse_tags_tid, cache_tid) do
    format_standard(
      metric,
  ...
  defp format({%Distribution{} = metric, tagged_series}, reverse_tags_tid, cache_tid) do
    ...
    distributions =
      Enum.map(tagged_series, fn {tags_id, {counts, sum, above_max, boundaries}} ->
        labels = resolve_labels(reverse_tags_tid, cache_tid, tags_id)
  ...
  defp format_standard({metric, series}, type, reverse_tags_tid, cache_tid) do
    name = format_name(metric.name)
    help = ["# HELP ", name, " ", escape_help(metric.description)]
    ...
    samples =
      Enum.map(series, fn {tags_id, value} ->
        case resolve_labels(reverse_tags_tid, cache_tid, tags_id) do
          nil ->
  ...
  defp resolve_labels(reverse_tags_tid, cache_tid, tags_id) do
    case :ets.lookup(cache_tid, tags_id) do
      ...
      [] ->
        ...
        formatted = do_format_labels(tags)
