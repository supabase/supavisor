defmodule Supavisor.Protocol.ParseMessageHandler do
  ...
  The query is rejected when it carries a session-level SET statement and the
  ...
  def handle_message(state, len, payload) do
    with :ok <- check_set_statements(state.set_statements_action, payload) do
  ...
  defp check_set_statements(:ignore, _payload), do: :ok

  defp check_set_statements(set_action, payload) do
    with [_name, rest] <- :binary.split(payload, <<0>>),
         [query, _] <- :binary.split(rest, <<0>>),
         {:ok, parsed} <- parse(query) do
      SetStatements.check(set_action, parsed, query)
    else
      _ -> :ok
    end
  end
