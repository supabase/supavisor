defmodule Supavisor.ProxyHandler do
  @moduledoc false
  ...
  def db_checkout(_, _, %{mode: :session, db_pid: {pool, db_pid}})
      when is_pid(pool) and is_pid(db_pid) do
    {pool, db_pid}
  end

  def db_checkout(_, :on_connect, %{mode: :transaction}), do: nil

  def db_checkout(query_type, _, data) when query_type in [:write, :read] do
    pool =
  ...
  def handle_db_pid(:transaction, _pool, nil), do: nil

  def handle_db_pid(:transaction, _pool, {pool, db_pid}) do
    Process.unlink(db_pid)
    :poolboy.checkin(pool, db_pid)
    nil
  end

  def handle_db_pid(:session, _, db_pid), do: db_pid
  ...
  def db_pid_meta({_, {_, pid}} = _key) do
  ...
  def handle_prepared_statements({_, pid}, bin, %{mode: :transaction} = data) do
    with {:ok, payload} <- Client.get_payload(bin),
  ...
  def handle_prepared_statements(_, _, _), do: nil

  @spec handle_actions(map) :: [{:timeout, non_neg_integer, atom}]
  def handle_actions(data) do
