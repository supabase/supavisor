defmodule Supavisor.PromExTest do
  ...
  setup_all do
    ...
    %{proxy: proxy, user: db_conf[:username], db_database: db_conf[:database]}
  end

  test "remove tenant tag upon termination", %{proxy: proxy, user: user, db_database: db_database} do
    ...
    Process.sleep(500)
    Supavisor.stop({{:single, @tenant}, user, :transaction, db_database})
    Process.sleep(500)
